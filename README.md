...
- `/` is the open-city TechTO dashboard. Its chat runs the live Backboard
  Planning Orchestrator with optional twin tools and specialist calls.
- `/techto` is the separate TechTO transit demonstration. It retains its
...
- `MONGODB_DATABASE` defaults to `techto`. If an older Atlas cluster still uses
  `twinto`, either set `MONGODB_DATABASE=twinto` or re-run
  `npm run mongo:bootstrap` against the new database name.
- `NEXT_PUBLIC_MAP_STYLE_URL` optionally overrides the MapLibre base style.
- Legacy `/twinto` URLs redirect to `/techto`.
