...
/** Result of layering the concert-surge extenuating-circumstances overlay (docs/twinto-implementation.md 2.5) on top of the selected candidate. */
  ...
  return (
    <GlassPanel className="flex h-full flex-col p-4" data-testid="stress-test-panel">
      <div className="flex items-center gap-2">
        <ShieldAlert className="h-4 w-4 text-twinto-accent" />
        <h3 className="text-sm font-semibold text-twinto-text">Stress Test</h3>
      </div>
        ...
        <div className="mt-3 flex-1 space-y-3 overflow-y-auto pr-1 twinto-scroll">
          ...
          {entry.result.invalidationReasons.length > 0 ? (
            <ul className="space-y-1.5">
              {entry.result.invalidationReasons.map((reason, index) => (
                <li key={index} className="flex items-start gap-1.5 text-xs text-twinto-error">
            ...
            <p className="flex items-start gap-1.5 text-xs text-twinto-teal">
          ...
          <div className="grid grid-cols-2 gap-2 text-center">
            <div className="rounded-lg border border-white/5 bg-white/[0.02] py-2">
              <p className="text-sm font-semibold text-twinto-text">
                {entry.result.baseline.metrics.meanWaitMinutes.toFixed(1)}m
              </p>
              <p className="text-[10px] uppercase tracking-wide text-twinto-muted">wait, no event</p>
            </div>
            <div className="rounded-lg border border-white/5 bg-white/[0.02] py-2">
              <p className="text-sm font-semibold text-twinto-text">
                {entry.result.stressed.metrics.meanWaitMinutes.toFixed(1)}m
              </p>
              <p className="text-[10px] uppercase tracking-wide text-twinto-muted">wait, under stress</p>
            </div>
          </div>
        </div>
      )}
