    ...
    <GlassPanel className="flex h-full flex-col p-4" data-testid="final-recommendation">
      ...
      {!result ? (
        ...
        <motion.div
          ...
          className="mt-3 flex-1 space-y-3 overflow-y-auto pr-1 twinto-scroll"
        >
          {result.recommendationOverridden && (
            <div className="flex items-start gap-2 rounded-lg border border-twinto-error/30 bg-twinto-error/[0.06] px-3 py-2">
              <ShieldAlert className="mt-0.5 h-3.5 w-3.5 shrink-0 text-twinto-error" />
              <p className="text-xs text-twinto-error">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-sm font-semibold text-twinto-text">{result.effectiveRecommendation.headline}</span>
          ...
          {result.effectiveRecommendation.tradeoffs.length > 0 && (
            <div>
              <p className="text-[11px] uppercase tracking-wide text-twinto-muted">Tradeoffs</p>
              <ul className="mt-1 space-y-1">
                {result.effectiveRecommendation.tradeoffs.map((tradeoff, index) => (
                  <li key={index} className="text-xs text-twinto-text/80 before:mr-1.5 before:text-twinto-accent before:content-['•']">
                    {tradeoff}
                  </li>
                ))}
              </ul>
          ...
          <p className="text-[11px] leading-relaxed text-twinto-muted">
