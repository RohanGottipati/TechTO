...
import { WaitTimeChart, type WaitTimeChartEntry } from "@/components/twinto/WaitTimeChart";
...
const METRIC_ROWS: { key: keyof TwinTORunResult["simulations"][number]["result"]["metrics"]; label: string; suffix?: string }[] = [
  ...
  return (
    <GlassPanel className="flex h-full flex-col p-4" data-testid="policy-comparison">
      <div className="flex items-center gap-2">
        <Scale className="h-4 w-4 text-twinto-accent" />
        <h3 className="text-sm font-semibold text-twinto-text">Policy Comparison</h3>
      </div>
      ...
      {!selected ? (
        ...
        <div className="mt-3 flex-1 overflow-y-auto twinto-scroll">
          <table className="w-full text-xs">
            <thead>
              <tr className="text-left text-[10px] uppercase tracking-wide text-twinto-muted">
            ...
            <tbody>
              {METRIC_ROWS.map((row) => (
                <tr key={row.key} className="border-t border-white/5">
                  <td className="py-1.5 text-twinto-muted">{row.label}</td>
                  <td className="py-1.5 text-right text-twinto-text">
