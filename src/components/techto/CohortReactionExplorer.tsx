    ...
    <GlassPanel className="flex h-full flex-col p-4" data-testid="cohort-reactions">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <Users className="h-4 w-4 text-twinto-accent" />
        ...
        <div className="mt-3 flex-1 space-y-3 overflow-y-auto pr-1 twinto-scroll">
          <div className="grid grid-cols-2 gap-2 text-center">
            <div className="rounded-lg border border-white/5 bg-white/[0.02] py-2">
              <p className="text-lg font-semibold text-twinto-text">
                {(entry.result.aggregate.populationWeightedAcceptance * 100).toFixed(0)}%
              </p>
              <p className="text-[10px] uppercase tracking-wide text-twinto-muted">population-weighted acceptance</p>
            </div>
            <div className="rounded-lg border border-white/5 bg-white/[0.02] py-2">
              <p className="text-lg font-semibold text-twinto-text">
                ±{entry.result.aggregate.stdDevAcceptance.toFixed(2)}
              </p>
              <p className="text-[10px] uppercase tracking-wide text-twinto-muted">std. dev across cohorts</p>
            </div>
          </div>

          <p className="text-[11px] text-twinto-muted">
          ...
          <ul className="space-y-2">
            {entry.result.reactions.map((reaction) => (
              <li key={reaction.cohortId} className="rounded-lg border border-white/5 bg-white/[0.02] px-3 py-2">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-xs font-medium text-twinto-text">{reaction.cohortId}</span>
                  <span
                    className={
                      reaction.acceptance >= 0.6
                        ? "text-xs font-semibold text-twinto-teal"
                        : reaction.acceptance <= 0.4
                          ? "text-xs font-semibold text-twinto-error"
                          : "text-xs font-semibold text-twinto-amber"
                    }
                  >
                    {(reaction.acceptance * 100).toFixed(0)}%
                  </span>
                </div>
                <p className="mt-1 text-[11px] leading-relaxed text-twinto-muted">{reaction.rationale}</p>
              </li>
            ))}
          </ul>
        </div>
